...
Our key insight is that neural network activations cluster into predictable ranges (Figure 4), allowing us to index weights by their input value ranges and load only necessary weight groups from NVMe storage (Figure 1).

\textbf{Contributions:}
\begin{itemize}
    \item Range-based indexing: 1.2GB metadata for 70B models
    \item GPU-accelerated parallel range checking: 700,000x speedup (Figure 5)
    \item Intelligent caching: 85-95\% hit rates after warmup (Figure 3)
...
Stores weights in system RAM, transferring them to GPU on demand. While maintaining accuracy, this approach achieves only 0.5 tokens/second \cite{llamacpp}, making real-time interaction impossible (Figure 2).

\subsection{Our Contribution}
We propose a fundamentally different approach: instead of compressing weights, we compress the \textit{access pattern}. Our key observation is that neural network activations are not random—they cluster into predictable ranges by layer and attention head (Figure 4). This enables us to index weights by their expected input ranges, loading only relevant weight groups from NVMe storage (Figure 1).

\section{Key Insight: Activation Clustering}

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/activation_distributions.pdf}
\caption{Activation distributions across Llama 2 layers. Early layers show near-uniform distributions, middle layers exhibit clustering, and late layers are highly peaked. This clustering property enables effective range-based indexing.}
\label{fig:distributions}
\end{figure}

Our method is built on a critical observation about neural network behavior: activations are not uniformly distributed. Analysis of Llama 2 7B reveals (Figure 4):
...
Additionally, temporal locality exists during autoregressive generation—recently used weights are likely to be used again, enabling effective caching (Figure 3).
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/gpu_speedup.pdf}
\caption{GPU kernel speedup comparison. Our parallel GPU kernel achieves 700,000x speedup over CPU sequential implementation, reducing range check time from 70 seconds to 0.0001 seconds per token.}
\label{fig:gpu_speedup}
\end{figure}

The key innovation enabling real-time performance is our GPU kernel that processes all inputs in parallel (Figure 5). For $d = 4096$ inputs (typical hidden dimension):
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/cache_warmup.pdf}
\caption{Cache warmup curve for 70B model. Hit rate improves from 45\% to 94\% over 1000 tokens, reaching usable speeds (80\% hit rate) after approximately 50 tokens.}
\label{fig:cache_warmup}
\end{figure}

We implement an LRU cache with predictive prefetching (Figure 3):
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/group_size_sensitivity.pdf}
\caption{Group size sensitivity analysis. Optimal group size (g=1000) balances metadata overhead (1.2GB) and cache hit rate (85\%).}
\label{fig:groupsize}
\end{figure}

Based on activation distribution analysis (Figure 6), we employ a hybrid strategy:
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/vram_comparison.pdf}
\caption{VRAM requirements across methods. RIWC enables 70B models on 12GB consumer GPUs, while full precision requires 140GB and quantization requires 35GB.}
\label{fig:vram}
\end{figure}
...
\caption{Throughput comparison for 70B model. RIWC achieves 15 tokens/second on 12GB VRAM, 30x faster than CPU offloading and 2x faster than INT4 quantization while maintaining 100\% accuracy.}
\label{fig:speed}
\end{figure}
...
Cache hit rate improves rapidly during conversation (Figure 3):
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/latency_breakdown.pdf}
\caption{End-to-end latency breakdown with warm cache. NVMe loading (60\%) and matrix multiplication (16\%) dominate, while range checking is negligible (0.04\%).}
\label{fig:latency}
\end{figure}
...
\textbf{GPU Kernel Impact (Figure 5):}
\begin{itemize}
...
\textbf{Group Size Sensitivity (Figure 6):}
\begin{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{GPU-accelerated range checking:} 700,000x speedup over CPU implementations (Figure 5)
    \item \textbf{Intelligent caching:} 85-95\% hit rates after conversation warmup (Figure 3)
    \item \textbf{Open-source implementation:} \texttt{github.com/yourusername/torch-range-indexed}
\end{enumerate}

RIWC represents a paradigm shift: instead of compressing weights, we compress the access pattern. This approach maintains full accuracy while dramatically reducing hardware requirements (Figure 8), democratizing access to large language models.
